...
Detector::Detector(const G4GDMLParser &parser) : G4VUserDetectorConstruction(), fParser(parser), World(fParser.GetWorldVolume())
{
  ...
  LAr_MPT->AddConstProperty("SCINTILLATIONYIELD", /*2500.0*/ 25 / MeV); // reduzido em 10x - photons/energia
  ...
  // creating the LAr-steel MPT -------------------------------------------------------------------------------------------------------
  surface_lar_steel_MPT = new G4MaterialPropertiesTable();

  G4double lar_steel_photon_energies[2] = {2.0 * eV, 2.341 * eV};
  G4double lar_steel_reflectivity[2] = {0.3, 0.3};
  surface_lar_steel_MPT->AddProperty("REFLECTIVITY", lar_steel_photon_energies, lar_steel_reflectivity, 2);

  // creating the LAr-al2o3 surface MPT -------------------------------------------------------------------------------------------------------
  surface_lar_al_MPT = new G4MaterialPropertiesTable();

  G4double lar_al_photon_energies[2] = {2.0 * eV, 2.341 * eV};
  G4double lar_al_reflectivity[2] = {0.7, 0.7};
  surface_lar_al_MPT->AddProperty("REFLECTIVITY", lar_al_photon_energies, lar_al_reflectivity, 2);
}

Detector::~Detector()
{
  delete LAr_MPT;
  delete surface_lar_al_MPT;
  delete surface_lar_steel_MPT;
  delete World;
}

G4VPhysicalVolume *FindVolume(G4VPhysicalVolume *pMother, G4String target)
{
  // iterating over the daughter volumes until the target is found
  auto n_daughters = (int)pMother->GetLogicalVolume()->GetNoDaughters();
...
{
  // finding the cryostat volume the LAr object
  auto detEnclosure = World->GetLogicalVolume()->GetDaughter(0);
  auto cryostat = FindVolume(detEnclosure, "Cryostat");

  // set the LAr property table
  auto TPC = FindVolume(cryostat, "TPC");
  auto LAr = TPC->GetLogicalVolume()->GetMaterial();
  LAr->SetMaterialPropertiesTable(LAr_MPT);
  LAr->GetIonisation()->SetBirksConstant(0.0000001 * mm / MeV);

  // configuring the LAr-Al surface
  G4OpticalSurface *surface_lar_al = new G4OpticalSurface("LArAl_surface");
  surface_lar_al->SetModel(unified);
  surface_lar_al->SetType(dielectric_metal);
  surface_lar_al->SetFinish(polished);
  surface_lar_al->SetMaterialPropertiesTable(surface_lar_al_MPT);

  // configuring the LAr-Steel surface
  G4OpticalSurface *surface_lar_steel = new G4OpticalSurface("LArSteel_surface");
  surface_lar_steel->SetModel(unified);
  surface_lar_steel->SetType(dielectric_metal);
  surface_lar_steel->SetFinish(polished);
  surface_lar_steel->SetMaterialPropertiesTable(surface_lar_steel_MPT);

  // assgning surfaces to volumes
  auto pCathode = FindVolume(cryostat, "Cathode");
  auto lCathode = pCathode->GetLogicalVolume();
  new G4LogicalSkinSurface("TPC_Cathode_surface", lCathode, surface_lar_steel);

  auto pSheel = FindVolume(cryostat, "SteelShell");
  auto lSheel = pSheel->GetLogicalVolume();
  new G4LogicalSkinSurface("TPC_Shell_surface", lSheel, surface_lar_steel);

  auto pShaper = FindVolume(cryostat, "FieldShaper");
  auto lShaper = pShaper->GetLogicalVolume();
  new G4LogicalSkinSurface("TPC_Shaper_surface", lShaper, surface_lar_al);
